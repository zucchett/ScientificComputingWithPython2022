...
# with $h=2/N$ the width of each of the $N$ slices the domain is divided into and 
# where
# $y_k$ is the value of the function at the $k-$th slice.

# (a) Write a program to compute the integral with $N=100$. How does the result 
# compare to the true value?

# (b) How much can $N$ be increased if the computation needs to be run in less 
# than a second? What is the gain in running it for 1 minute? Use `timeit` to 
# measure the time.
...
def integral(N, upper_limit, lower_limit, f):
    result = 0
    h = (upper_limit - lower_limit) / N # 2/N
    for i in range(N):
        result = result + h*f(i)
    return phase(result)
...
print("Execution time for integral: "+ str(endtime) + " seconds")
